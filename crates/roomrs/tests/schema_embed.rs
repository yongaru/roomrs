...
fn embedded_snapshot_roundtrip() {
    ...
    assert_eq!(roomrs::decompress_snapshot(&comp).unwrap(), json.as_bytes(), "압축 왕복 무손실");

    // 런타임 구성 EmbeddedSchema (테스트 전용 leak) — snapshot() 파스 왕복
    let embedded = roomrs::EmbeddedSchema { version: 1, compressed: Box::leak(comp.into_boxed_slice()) };
    let back = embedded.snapshot().unwrap();
    assert_eq!(back, snap);
    assert_eq!(back.hash(), snap.hash());

    // 파손 blob = Migration 에러 (한국어)
    let bad = roomrs::EmbeddedSchema { version: 9, compressed: b"\xff\x00corrupt" };
    match bad.snapshot() {
...
fn diff_plan_via_facade() {
    ...
    assert_eq!(plan.safe, vec![r#"ALTER TABLE "items" ADD COLUMN "name" TEXT"#.to_string(), r#"CREATE INDEX "idx_items_name" ON "items"("name")"#.to_string(),], "안전 연산: nullable ADD COLUMN + CREATE INDEX");
    ...
    assert!(sql.contains(r#"ALTER TABLE "items" ADD COLUMN "name" TEXT;"#), "{sql}");
...
fn snapshot_dir_scan_via_facade() {
    ...
    v1.write_to(&roomrs::snapshot_path(dir.path(), "app_db", 1)).unwrap();
    v2.write_to(&roomrs::snapshot_path(dir.path(), "app_db", 2)).unwrap();
    // 무관 파일 — 무시
    v1.write_to(&roomrs::snapshot_path(dir.path(), "other_db", 1)).unwrap();
    ...
    assert!(roomrs::list_snapshot_versions(&dir.path().join("없음"), "app_db").unwrap().is_empty());
    ...
    if std::env::var("ROOMRS_SCHEMA_DIR").is_err() {
        assert_eq!(roomrs::resolve_schema_dir("/proj"), std::path::Path::new("/proj").join(roomrs::SCHEMA_DIR_RELATIVE));
    }
}
