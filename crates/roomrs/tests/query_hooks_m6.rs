...
fn seed() -> (tempfile::TempDir, Db) {
    ...
    let db = Db::builder().sqlite(dir.path().join("q.db")).build().unwrap();
    let h = db.run_sync();
    for (t, y) in [("가", 2001), ("나", 2005), ("다", 2010)] {
        h.book_dao().add(&Book { id: 0, title: t.into(), year: y }).unwrap();
    }
    (dir, db)
}
...
fn builder_sync_execution() {
    ...
    let rows: Vec<Book> = Query::select::<Book>().and_where(col("year").ge(2005i64)).order_by("year", Order::Desc).fetch_all(db.run_sync()).unwrap();
    assert_eq!(rows.len(), 2);
    assert_eq!(rows[0].year, 2010);

    let one: Book = Query::select::<Book>().and_where(col("title").eq("나")).fetch_one(db.run_sync()).unwrap();
    assert_eq!(one.year, 2005);

    let none: Option<Book> = Query::select::<Book>().and_where(col("year").gt(9999i64)).fetch_optional(db.run_sync()).unwrap();
    assert!(none.is_none());

    let cnt: i64 = Query::count::<Book>().and_where(col("year").in_list([2001i64, 2010])).fetch_scalar(db.run_sync()).unwrap();
    assert_eq!(cnt, 2);
}
...
fn builder_handle_symmetry() {
    let (_d, db) = seed();
    let q = Query::select::<Book>().and_where(col("year").ge(2005i64)).order_by("id", Order::Asc);

    let sync_rows: Vec<Book> = q.clone().fetch_all(db.run_sync()).unwrap();
    let async_rows: Vec<Book> = smol::block_on(async { q.fetch_all(db.run_async()).await }).unwrap();
...
fn builder_schema_validation() {
    let (_d, db) = seed();
    let r: roomrs::Result<Vec<Book>> = Query::select::<Book>().and_where(col("ghost").eq(1i64)).fetch_all(db.run_sync());
...
fn escaped_like_matches_literal_wildcards() {
    ...
    for title in ["100%", "under_score", "bang!mark", "한%"] {
        h.book_dao().add(&Book { id: 0, title: title.into(), year: 2020 }).unwrap();
    }

    for (pattern, escape, expected) in [("100!%", '!', "100%"), ("under!_score", '!', "under_score"), ("bang!!mark", '!', "bang!mark"), ("한界%", '界', "한%")] {
        let rows: Vec<Book> = Query::select::<Book>().and_where(col("title").like_escaped(pattern, escape)).fetch_all(*h).unwrap();
...
fn operational_hooks() {
    ...
    // 기본 풀 크기 = cores.clamp(1,4)+1 (결정 51: live worker 전용 연결 없음)
    let cores = std::thread::available_parallelism().map(|n| n.get()).unwrap_or(1);
    let connections_per_open = cores.clamp(1, 4) + 1;
    assert_eq!(opens.load(AO::SeqCst), connections_per_open as u64, "on_open은 모든 내부 연결마다 호출");

    let before = logs.load(AO::SeqCst);
    db.run_sync().book_dao().add(&Book { id: 0, title: "로그".into(), year: 1 }).unwrap();
    ...
    assert_eq!(opens.load(AO::SeqCst), connections_per_open as u64 * 2, "on_open은 매 오픈의 모든 내부 연결마다 호출");
}
