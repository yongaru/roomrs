...
fn text_primary_key_is_not_null() {
    assert!(TextKey::DDL[0].contains("\"id\" TEXT PRIMARY KEY NOT NULL"), "DDL: {}", TextKey::DDL[0]);
}
...
fn aliased_duplicate_entity_table_is_rejected() {
    ...
    assert!(matches!(result, Err(roomrs::Error::Config(message)) if message.contains("테이블 이름 중복")));
}
