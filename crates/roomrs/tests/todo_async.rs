...
async fn crud_roundtrip(dir: &tempfile::TempDir) -> roomrs::Result<()> {
    let db = Db::builder().sqlite(dir.path().join("a.db")).migrate(MigrationPolicy::Auto).build_async().await?;
    let h = db.run_async();
    let dao = h.todo_dao();

    let id = dao.add(&Todo { id: 0, title: "비동기".into(), done: false }).await?;
    assert!(id > 0);
    ...
    use DbTxDaos as _;
    h.transaction(|tx| {
        tx.todo_dao().add(&Todo { id: 0, title: "tx".into(), done: false })?;
        Ok(())
    })
    .await?;
    let cnt: i64 = h.query_scalar("SELECT COUNT(*) FROM todos", ()).await?;
...
fn executor_tokio() {
    ...
    tokio::runtime::Builder::new_multi_thread().build().unwrap().block_on(crud_roundtrip(&dir)).unwrap();
}
...
fn futures_are_send_spawnable() {
    ...
    rt.block_on(async {
        let db = Db::builder().sqlite(dir.path().join("s.db")).build_async().await.unwrap();
        let handle = tokio::spawn(async move {
            let h = db.run_async();
            h.todo_dao().add(&Todo { id: 0, title: "spawned".into(), done: false }).await
        });
...
fn generated_dao_future_is_directly_spawnable() {
    ...
    rt.block_on(async {
        let db = Db::builder().sqlite(dir.path().join("direct-spawn.db")).build_async().await.unwrap();
...
fn panicking_job_is_isolated() {
    ...
    futures::executor::block_on(async {
        let db = Db::builder().sqlite(dir.path().join("p.db")).migrate(MigrationPolicy::Auto).build_async().await.unwrap();
        let h = db.run_async();
        ...
        // 풀 생존 확인 — 이후 정상 동작
        let id = h.todo_dao().add(&Todo { id: 0, title: "생존".into(), done: false }).await.unwrap();
        assert!(id > 0);
    });
}
...
fn tokio_panicking_job_is_isolated() {
    ...
    tokio::runtime::Builder::new_multi_thread().build().unwrap().block_on(async {
        let db = Db::builder().sqlite(dir.path().join("tp.db")).migrate(MigrationPolicy::Auto).build_async().await.unwrap();
        let h = db.run_async();
...
fn tokio_cancelled_future_is_safe() {
    let dir = tempfile::tempdir().unwrap();
    tokio::runtime::Builder::new_multi_thread().build().unwrap().block_on(async {
        let db = Db::builder().sqlite(dir.path().join("tc.db")).build_async().await.unwrap();
        let h = db.run_async();

        // poll 없이 즉시 drop — 작업 미제출, 아무 일도 안 일어남
        drop(h.todo_dao().add(&Todo { id: 0, title: "취소0".into(), done: false }));

        // 첫 poll 후 drop — 작업은 제출됐고 워커에서 완료, 결과만 폐기
        {
            let dao = h.todo_dao();
            let todo = Todo { id: 0, title: "취소1".into(), done: false };
        ...
        for _ in 0..200 {
            let cnt: i64 = h.query_scalar("SELECT COUNT(*) FROM todos", ()).await.unwrap();
            if cnt == 1 {
                landed = true;
                break;
            }
...
fn cancelled_future_is_safe() {
    let dir = tempfile::tempdir().unwrap();
    smol::block_on(async {
        let db = Db::builder().sqlite(dir.path().join("c.db")).build_async().await.unwrap();
        let h = db.run_async();

        // poll 없이 즉시 drop — 시작 전 취소
        drop(h.todo_dao().add(&Todo { id: 0, title: "취소1".into(), done: false }));
