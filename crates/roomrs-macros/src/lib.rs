...
/// Maps a struct to a SQLite table (specification §5.1).
///
/// Field attributes include `#[pk]`, `#[pk(autoincrement)]`, `#[column(...)]`,
/// and `#[json]`. Entity attributes include `table`, `primary_key(...)`,
/// `unique(...)`, `index(...)`, `foreign_key(...)`, `check`, and `trigger`.
...
/// which is why an explicit schema export must be followed by `cargo build`;
/// that rebuild picks the new file up.
#[proc_macro_attribute]
pub fn database(args: TokenStream, input: TokenStream) -> TokenStream {
    into_tokens(database::expand(args.into(), input.into()))
}

/// Hidden schema-export harness entry point generated by `#[database]`.
///
/// This attribute is emitted only for the isolated `cfg(roomrs_export)` build
/// started by `cargo roomrs`. It maps the generated function onto Rust's
/// stable test harness so schema discovery does not run the application's
/// `main`.
#[doc(hidden)]
#[proc_macro_attribute]
pub fn roomrs_export(args: TokenStream, input: TokenStream) -> TokenStream {
    if !args.is_empty() {
        return syn::Error::new(proc_macro2::Span::call_site(), "roomrs_export는 인자를 받지 않습니다").to_compile_error().into();
    }
    let item = match syn::parse::<syn::ItemFn>(input) {
        Ok(item) => item,
...
fn migrations_dir_impl(input: proc_macro2::TokenStream) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let rel = lit.value();
    let manifest = std::env::var("CARGO_MANIFEST_DIR").map_err(|_| syn::Error::new(lit.span(), "CARGO_MANIFEST_DIR 없음"))?;
...
fn scan_migration_files(dir: &std::path::Path, span: proc_macro2::Span) -> syn::Result<Vec<(u32, u32, String)>> {
    let entries = std::fs::read_dir(dir).map_err(|e| syn::Error::new(span, format!("마이그레이션 디렉터리를 읽을 수 없습니다 ({}): {e}", dir.display())))?;
    ...
    for entry in entries {
        ...
        let (Some(f), Some(t)) = (parts.next(), parts.next()) else {
            return Err(syn::Error::new(span, format!("파일명 규칙 위반: {name}.sql — {{from}}_{{to}}_이름.sql 형식 필요")));
        };
        let (Ok(from), Ok(to)) = (f.parse::<u32>(), t.parse::<u32>()) else {
            return Err(syn::Error::new(span, format!("파일명 버전 파싱 실패: {name}.sql — 숫자_{{from}}_{{to}} 필요")));
        };
        // 선행 0 금지 — "01"과 "1"이 같은 버전으로 중복될 수 있다.
        // 스냅샷 파일명 정책과 동일 규칙 (L-15)
        if (f.len() > 1 && f.starts_with('0')) || (t.len() > 1 && t.starts_with('0')) {
            return Err(syn::Error::new(span, format!("파일명 버전 선행 0 금지: {name}.sql — \"01\" 대신 \"1\" 을 사용하세요")));
        }
        // 다운그레이드/자기참조 금지 (L-15)
        if from >= to {
            return Err(syn::Error::new(span, format!("마이그레이션 버전 역행: {name}.sql — from({from}) < to({to}) 여야 합니다")));
        }
    ...
    for w in files.windows(2) {
        if w[0].0 == w[1].0 {
            return Err(syn::Error::new(span, format!("중복 from 버전: {} — 같은 버전에서 출발하는 마이그레이션이 2개 이상입니다 ({}_{} / {}_{})", w[0].0, w[0].0, w[0].1, w[1].0, w[1].1)));
        }
    }
    Ok(files)
}
